...
Sometimes it is helpful to make a sequence of values in a vector.  `R` has some built-in functionality here for that.
...
Data within vectors can be subjected to unary operators.
...
Creating matrices is done columnwise; if you want them to be rowwise, you have to ask for it.
...
Arithmetic operators on matrices work the same way (as long as they are matrices of the proper number of rows and columns).
...
Notice that lists use two sets of square brackets instead of one—to differentiate themselves from a normal vector
...
This is because *technically*, the first element in the list is also a list and what we are trying to get from that is the first element inside that contained list.  
...
Here is a quick correlation between the sepal and petal lengths in some `iris` data set.
...
Let's consider the following data as individual vectors.
...
The `leaflet` map package is one way to make a quick interactive map based on raw *Latitude* and *Longitude* columns.
...
beetles %>%
  leaflet() %>%
  addProviderTiles(provider = leaflet::providers$Esri.WorldTopo) %>%

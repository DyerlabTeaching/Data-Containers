...
> Data needs to be contained within objects so that we can examine, manipulate, sort, analyze, and communicate about it. In this topic, we examine some of the more basic data containers in `R`, though in your practice, you will most likely use `data.frame` objects for almost all data interaction and lists as they are the default output from most statistical analyses.  
...
There are a lot of times when we require a sequence of values and it would get a bit tedious to type them all out manually. `R` has several options for creating vectors that are comprised of a sequence of values.
...
However, it is only available to make a sequence where the increment from one value to the next is `1`.

```{r}
3.2:5.7
```

For more fine-grained control, we can use the function `seq()` to iterate across a range of values and specify either the step size (here from 1-10 by 3s)
...
To access and change values within a vector, we use square brackets and the number of the entry of interest. It should be noted that in `R`, the first element of a vector is \# 1.
...
In addition to getting the values from a vector, assignment of individual values proceeds similarly.
...
If you assign a value to a vector that is way off the end, it will fill in the intermediate values with `NA` for you.
...
If the two vectors are not multiples of each other in length, it will still recycle the shorter one but will also give you a warning that the two vectors are not conformant (just an FYI).
...
The operations used are dependent upon the base data type. For example, the following `character` values can be passed along to the `paste()` function to put each of the elements in the first vector with the corresponding values in the second vector (and specifying the separator).
...
A `matrix` is a 2-dimensional container for the same kind of data as well. The two dimensions are represented as *rows* and *columns* in a rectangular configuration. Here I will make a 3x3 matrix consisting of a sequence of numbers from 1 to 9.
...
Binary operators using the normal operators in the top row of your keyboard are generally element-wise operations. Here the addition of these two matrices requires:\
...
However, there is another kind of matrix multiplication that sums the product of rows and columns. Since this is also a variety of multiplication but is carried out differently, we need to use a different operator. Here the matrix multiplication operator is denoted as the combination of characters `%*%`.
...
4.  The resulting element at the $i$ $j$ position is the sum of the multiplication of the elements in the $i^{th}$ row of the left matrix and the $j^{th}$ column of the right one.
...
Lists can be more valuable if we use names for the *keys* instead of just numbers. Here, I make an empty list and then assign values to it using names (as character values) in square brackets.
...
In addition to the square bracket approach, we can also use \$ notation to add elements to the list (like shown above).

```{r}
myInfo$Vegetarian <- FALSE
```
...
This can be helpful at times when you did not create the list yourself and want to see what is inside of it.
...
Would give you an error (if I ran the chunk but I cannot because it is an error and won't let me compile this document if I do). For names that have spaces, we need to enclose them inside back-ticks (as shown in the output above).
...
Here is an example looking at some data from three species of Iris on the lengths and widths of sepals and petals. The data look like:
...
We can look at the correlation between two variables using the built-in `cor.test()` function.
...
In fact, the contents of the output are just keys and values, even though when we printed it all out, it was formatted as a much more informative output.
...
Notice how the function tells us a few things about the data.

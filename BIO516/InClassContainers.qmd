...
```{r}
people <- c( "Rodney", "Sarah", "Elizabeth", "Colin" ) 
people

person <- "Bob"
person

myFavoriteLetters <- LETTERS[1:26]
myFavoriteLetters
...
```{r}
matrix( 0, nrow=2, ncol=3) -> zeroMat
zeroMat

matrix( c(1,2,3,4), nrow = 2 ) -> X
...
```{r}
cat <- list("Name" = "Colin", chonk = TRUE, legs = 4  )
cat[["Feeding"]] <- c("Morning", "Noon", "Night", "A little milk please")
cat$Snacks <- c("yes", "please")
cat 
cat$chonk
